...
export const SERVER_INSTRUCTIONS = [
  ...
  "render_component 会在内存中编译未发布工程并使用 fairygui-dom runtime-preview；state.controllers/state.scrolls 可设置仅用于本次截图的控制器页和滚动位置，它不是 Unity 像素真值。"
].join("\n");
...
const TOOL_DESCRIPTIONS: Record<FairyGuiToolName, string> = {
  ...
  "fairygui.render_component":
    "在内存中编译未发布工程，用隔离 FairyGUI-dom runtime 渲染并返回 PNG；可按受限选择器临时设置当前截图的控制器页和滚动位置且不写盘。",
  "fairygui.validate":
